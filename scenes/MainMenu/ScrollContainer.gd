...
func inputEvent( ev ):
	...
	if (ev is InputEventMouseButton)and(ev.pressed == false):
		...
		var tween = Tween.new()
		add_child(tween)
		tween.interpolate_method(self, "set_h_scroll", get_h_scroll(), get_h_scroll()-2*swypeDX, 0.5, Tween.TRANS_LINEAR, Tween.EASE_IN_OUT)
		tween.interpolate_callback(tween, 0.5, "queue_free")
		tween.start()
		swypePoint = null
	if swype and (ev is InputEventMouseMotion):
		set_h_scroll(get_h_scroll() - ev.position.x + swypePoint)
